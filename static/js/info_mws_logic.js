  ...
  if (attachments && attachments.length > 0) {
    ...
    attachments.forEach((att) => {
      const deleteButtonHtml = userIsAdmin
        ? `<button onclick="deleteAttachment('${partId}', '${att.stored_filename}')" class="text-red-500 hover:text-red-700 text-xs font-semibold" title="Hapus Lampiran">
             <i class="fas fa-trash"></i>
           </button>`
        : "";

      const elementId = `attachment-item-${att.stored_filename.replace(/\./g, "-")}`;
 ...
 */
window.deleteAttachment = async function (partId, storedFilename) {
  ...
  try {
    const response = await fetch(`/delete_attachment/${partId}/${storedFilename}`, {
      method: "DELETE",
      headers: { "X-CSRFToken": csrfToken },
    });
    ...
    if (result.success) {
      showToast(result.message, "success");
      const elementId = `attachment-item-${storedFilename.replace(/\./g, "-")}`;
...
function displayStrippingReportInTable(data) {
  ...
  if (data.length === 0) {
    const colspan = userIsAdmin ? 11 : 10;
...
window.openAllStrepModal = async function () {
  ...
  button.disabled = true;
  tableBody.innerHTML = `<tr><td colspan="25" class="text-center py-8"><i class="fas fa-spinner fa-spin text-2xl text-gray-500"></i></td></tr>`;
  ...
  try {
    ...
    if (result.success) {
      displayAllStrepInModalTable(result.data);
    } else {
      showToast(`Error: ${result.error}`, "error");
      tableBody.innerHTML = `<tr><td colspan="25" class="text-center py-4 text-red-500">${result.error}</td></tr>`;
    }
  } catch (error) {
    console.error("Terjadi kesalahan jaringan:", error);
    showToast("Terjadi kesalahan jaringan saat mengambil data ALL Strep.", "error");
    tableBody.innerHTML = `<tr><td colspan="25" class="text-center py-4 text-red-500">Gagal memuat data.</td></tr>`;
  } finally {
...
function displayAllStrepInModalTable(data) {
  const tableBody = document.getElementById("all-strep-table-body");
  if (!tableBody) return;
  ...
  if (data.length === 0) {
    tableBody.innerHTML = `<tr><td colspan="25" class="text-center py-4 text-gray-500">Data tidak ditemukan</td></tr>`;
  } else {
    data.forEach((item) => {
      const row = `
        <tr class="hover:bg-gray-50">
          <td class="px-2 py-2 whitespace-nowrap text-sm text-gray-700 font-medium">${item.part_id || "-"}</td>
          ...
          <td class="px-2 py-2 whitespace-nowrap text-sm text-gray-500">${item.time_stripping_work_days || "-"}</td>
...
window.saveStrippingEdit = async function (strepId, button) {
  ...
  inputs.forEach((input) => {
    if (input.value) formData[input.dataset.field] = input.value;
  });
  const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute("content");
  const partId = document
    .getElementById("get-stripping-report-btn")
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("add-stripping-btn")?.addEventListener("click", () => addStrippingRow());
});
